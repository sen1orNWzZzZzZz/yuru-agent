...
PlannerAgent.plan() ── 解析需求，协调执行
  ...
  ├── HotelAgent ───────── 数据库查询酒店 + 小红书口碑
  ├── RestaurantAgent ──── 数据库查询餐厅 + 特色菜品
  ├── AttractionAgent ──── 数据库查询景点 + 高反风险评估
...
│   │   ├── llm_client.py            # 通用OpenAI格式LLM客户端
